...
export default function AnomalyList({ anomalies = [] }) {
  if (!anomalies.length) {
    return (
      <div className="anomaly-list__empty">
    ...
    <div className="anomaly-list">
      {anomalies.map((ano) => (
        <div key={ano.id} className="anomaly-list__row">
          ...
          {/* Icône type */}
          <div className={`anomaly-list__type-icon anomaly-list__type-icon--${ano.severite}`}>
            {TYPE_ICONS[ano.type] ?? "⚠"}
          </div>
          ...
          <div className="anomaly-list__body">
            ...
            <div className="anomaly-list__meta">
              {ano.documents.map((doc) => (
                <span key={doc} className="anomaly-list__doc-ref">{doc}</span>
              ))}
